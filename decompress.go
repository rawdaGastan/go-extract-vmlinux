...
func decompressData(data []byte, tmpFile *os.File, o algoOptions) error {
	...
	for i := 0; i < headerCount; i++ {
		...
		// close lzo reader after write
		if reader, ok := r.(*lzo.Reader); ok {
			defer reader.Close()
		}
...
func lZop(kernelStream io.Reader) (io.Reader, error) {
	return lzo.NewReader(kernelStream)
}
...
func TryDecompressKernel(kernelImagePath string, outputKernelPath string) error {
	...
	for _, algo := range algos {
		if err = decompressData(kernelData, tmpFile, algo); err == nil {
			...
			f, err := os.Create(outputKernelPath)
			if err != nil {
				return err
			}
			...
			log.Info().Str("Algorithm", algo.name).Str("kernel", kernelImagePath).Msg("Decompressed")
			return nil
		}
